...
export default function AdminPage() {
  const [selectedAction, setSelectedAction] = useState<'submit' | 'delete' | 'update'>('submit');
  ...
  return (
    <main className="absolute px-4 w-full left-0 top-20 flex flex-col items-center">
      ...
      <div className="flex w-full max-w-xl mt-8 gap-x-2 px-4">
        <Button onClick={() => setSelectedAction('submit')}>Submit</Button>
        <Button onClick={() => setSelectedAction('delete')}>Delete</Button>
        <Button onClick={() => setSelectedAction('update')}>Update</Button>
      </div>
